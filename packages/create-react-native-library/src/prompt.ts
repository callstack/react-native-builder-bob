...
export type Answers = Awaited<ReturnType<typeof prompt.show>>;
...
const getGitConfig = async (key: string): Promise<string> => {
  try {
...
export const prompt = create(['[name]'], {
  ...
  directory: {
    type: 'text',
    description: 'Directory to create the library in',
  ...
  authorName: {
    ...
    initial: async () => getGitConfig('user.name'),
    validate: (input) => Boolean(input) || 'Cannot be empty',
  ...
  authorEmail: {
    ...
    initial: async () => getGitConfig('user.email'),
    validate: (input) =>
      /\S+@\S+\.\S+/.test(input) || 'Must be a valid email address',
    skip: (): boolean => prompt.read().local === true,
  },
  authorUrl: {
    type: 'text',
    description: 'URL of the package author',
  ...
  languages: {
    ...
    prompt: () => {
      const answers = prompt.read();
      const choices: typeof LANGUAGE_CHOICES = LANGUAGE_CHOICES.filter(
        (choice) => {
          if (choice.types && typeof answers.type === 'string') {
  ...
  example: {
    ...
    prompt: () => {
      ...
      const choices: typeof EXAMPLE_CHOICES = EXAMPLE_CHOICES.filter(
        (choice) => {
          if (answers.type) {
  ...
  reactNativeVersion: {
    ...
    initial: SUPPORTED_REACT_NATIVE_VERSION,
    validate: (input) =>
      input === 'latest' ||
      /^\d+\.\d+\.\d+(-.+)?$/.test(input) ||
