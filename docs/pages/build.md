...
3. Add `"includesGeneratedCode": true` to the `codegenConfig` field in your `package.json`:
...
4. Add a `react-native.config.js` at the root with the correct `cmakeListsPath`:
   ...
   This makes sure that gradle will pickup the `CMakeLists.txt` file generated by the codegen script.

5. Add a gradle task to `example/android/app/build.gradle` to automatically run the codegen script when building the example app:
...
6. Add a `pre_install` hook to `example/ios/Podfile` to automatically run the codegen script when installing pods:
